...
export class AgentModelService {
  ...
  async generateCodeResponse(userPrompt: string): Promise<{ framework: string; code: any; otherResponse: string }> {
    const systemPrompt = `
      You are an expert frontend React developer operating in a sandpack environment. You will be given a description of a website from the user, and then you will return code for it using React and Tailwind CSS. Follow the instructions carefully, it is very important for my job.
    ...
    const response = await this.openai.beta.chat.completions.parse({
      ...
      max_tokens: 1200,
      temperature: 0.0,
      response_format: zodResponseFormat(this.generateCode, "output-schema-for-code-g"),
    });
